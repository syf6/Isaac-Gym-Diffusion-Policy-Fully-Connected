...
from ur3e_utils_v3 import quat_axis, orientation_error, cube_grasping_yaw, control_ik, control_osc, create_simulation, create_viewer, configure_ur3e_dof, create_assets, create_envs
...
def main():
    ...
    # Create assets
    table_asset, box_asset,_, ur3e_asset, pillar1_asset ,pillar2_asset ,pillar3_asset ,pillar4_asset ,destination_asset,sensor_idx = create_assets(gym, sim, asset_root, table_dims, box_size, ur3e_asset_file)
    ...
    envs, box_idxs,  pillar1_idxs,pillar2_idxs,pillar3_idxs,pillar4_idxs, hand_idxs, init_pos_list, init_rot_list ,destination_idxs, num_sensors, sensor= create_envs(
    ...
    while not gym.query_viewer_has_closed(viewer):
        ...
        for i in range(num_sensors):
            ...
            print("sensor_data.force",sensor_data.force)   # 打印力，单位为Vec3
            print("sensor_data.torque",sensor_data.torque)  # 打印扭矩，单位为Vec3

        # 使用张量API获取所有力传感器的数据
        fsdata = gym.acquire_force_sensor_tensor(sim)
        fsdata_torch = gymtorch.wrap_tensor(fsdata)
        gym.refresh_force_sensor_tensor(sim)
        forces = fsdata_torch[:, :3]  # 前三个值是力
        torques = fsdata_torch[:, 3:6]  # 后三个值是扭矩
        print("tensor force",forces)
        print("tensor torques",torques)
        ...
        try:
            ...
            if type == JS_EVENT_AXIS:
                ...
                if code == 1:  # 前後方向の軸
                    ...
                    goal_pos[:, 0] += value / 32767.0 * 0.006  # スケーリングして位置を更新
                elif code == 0:  #  左右方向の軸
                    print("Axis 1 moved to {value}")
                    goal_pos[:, 1] += value / 32767.0 * 0.006
                elif code == 3:  # 上下方向の軸
                    print("Axis 3 moved to {value}")
                    goal_pos[:, 2] += value / 32767.0 * 0.006
