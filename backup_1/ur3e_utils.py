...
def create_assets(gym, sim, asset_root, table_dims, box_size, ur3e_asset_file):
    ...
    pillar1_asset = create_pillar_asset(gym,sim,pillar_width = 0.01,pillar_depth = 0.01,pillar_height=0.3)
    pillar2_asset = create_pillar_asset(gym,sim,pillar_width = 0.01,pillar_depth = 0.01,pillar_height=0.3)
    pillar3_asset = create_pillar_asset(gym,sim,pillar_width = 0.01,pillar_depth = 0.01,pillar_height=0.3)
    pillar4_asset = create_pillar_asset(gym,sim,pillar_width = 0.01,pillar_depth = 0.01,pillar_height=0.3)
    ...
    return table_asset, box_asset, box2_asset ,ur3e_asset ,pillar1_asset, pillar2_asset, pillar3_asset, pillar4_asset
...
def add_pillar_in_create_envs(table_dims,box_size,pillar_pose, env, pillar_asset, i, gym , x,y):
    ...
    pillar_handle = gym.create_actor(env, pillar_asset, pillar_pose, "box", i, 0)
...
def create_envs(gym, sim, num_envs, num_per_row, spacing, table_asset, box_asset,pillar1_asset, pillar2_asset, pillar3_asset, pillar4_asset,ur3e_asset, ur3e_dof_props, default_dof_state, default_dof_pos, table_dims, box_size, viewer):
    ...
    for i in range(num_envs):
        ...
        box_pose.p.y = -0.3 #+ np.random.uniform(-0.2, 0.2) # -0.2
        ...
        # box2_pose.p.x = 0.4
        # box2_pose.p.y = 0.2 #+ np.random.uniform(-0.2, 0.2)
        ...
        # box2_handle = gym.create_actor(env, box2_asset, box2_pose, "box2", i, 0)
        # color2 = gymapi.Vec3(np.random.uniform(0, 1), np.random.uniform(0, 1), np.random.uniform(0, 1))
        # gym.set_rigid_body_color(env, box2_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, color2)
        
        # # get global index of box in rigid body state tensor
        # box2_idx = gym.get_actor_rigid_body_index(env, box2_handle, 0, gymapi.DOMAIN_SIM)
        # box_idxs.append(box2_idx)
    ...
    return envs, box_idxs,pillar1_idxs,pillar2_idxs,pillar3_idxs,pillar4_idxs, hand_idxs, init_pos_list, init_rot_list