...
# 模拟读手柄的函数
def read_joystick_from_csv(queue, csv_file_path):
    with open(csv_file_path, 'r') as csv_file:
        reader = csv.reader(csv_file)
        for row in reader:
            type, code, value = row
            type, code, value = int(type, 16), int(code), int(value)
...
def main():
    ...
    # 启动一个线程来读取CSV文件并模拟手柄输入
    csv_file_path = 'joystick_input.csv'  # CSV文件的路径
    joystick_thread = threading.Thread(target=read_joystick_from_csv, args=(joystick_queue, csv_file_path))
