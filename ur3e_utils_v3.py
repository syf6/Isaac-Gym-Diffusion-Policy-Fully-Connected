...
def create_assets(gym, sim, asset_root, table_dims, box_size, ur3e_asset_file):
    ...
    box_asset_options.density = 10000

    box_asset = gym.create_box(sim, box_size, box_size, box_size, box_asset_options)
    box2_asset = gym.create_box(sim, box_size, box_size, box_size, box_asset_options)
    ...
    # destination box
    destination_asset_options = gymapi.AssetOptions()
    destination_width =0.1
    destination_depth =0.1
    destination_height =0.01
    destination_asset = gym.create_box(sim, destination_width , destination_depth , destination_height ,destination_asset_options)
    ...
    # 获取destination asset上表面的刚体索引
    #top_surface_body_idx = gym.get_actor_rigid_body_index(destination_asset, 0, gymapi.DOMAIN_SIM)  # 直接使用索引0
    top_surface_body_idx = 0  # 直接使用索引0

    # 定义传感器的姿态，紧贴destination asset的表面
    sensor_pose = gymapi.Transform()
    sensor_pose.p = gymapi.Vec3(0, 0, destination_height)  # Z分量设置为0，紧贴表面
    sensor_pose.r = gymapi.Quat(0, 0, 0, 1)  # 单位四元数，表示无旋转

    # 在destination asset的上表面创建传感器
    sensor_idx = gym.create_asset_force_sensor(destination_asset, top_surface_body_idx, sensor_pose, sensor_props)

    # load ur3e asset
    ur3e_asset_options = gymapi.AssetOptions()
    ur3e_asset_options.armature = 0.01
    ur3e_asset_options.fix_base_link = True
    ur3e_asset_options.disable_gravity = True
    ur3e_asset_options.flip_visual_attachments = True
    ur3e_asset_options.use_mesh_materials = True
    ur3e_asset_options.mesh_normal_mode = gymapi.COMPUTE_PER_VERTEX
    ur3e_asset_options.vhacd_enabled = True
    ur3e_asset_options.convex_decomposition_from_submeshes = True
    ur3e_asset = gym.load_asset(sim, asset_root, ur3e_asset_file, ur3e_asset_options)
    
    return table_asset, box_asset, box2_asset , ur3e_asset ,pillar1_asset, pillar2_asset, pillar3_asset, pillar4_asset,destination_asset,sensor_idx 
...
def create_envs(gym, sim, num_envs, num_per_row, spacing, table_asset, box_asset,destination_asset,pillar1_asset, pillar2_asset, pillar3_asset, pillar4_asset,ur3e_asset, ur3e_dof_props, default_dof_state, default_dof_pos, table_dims, box_size, viewer):
    ...
    for i in range(num_envs):
        ...
        # 获取actor上附着的力传感器数量
        num_sensors = gym.get_actor_force_sensor_count(env, destination_handle)

        # 遍历所有传感器
        for i in range(num_sensors):
            sensor = gym.get_actor_force_sensor(env, destination_handle, i)
    ...
    return envs, box_idxs,pillar1_idxs,pillar2_idxs,pillar3_idxs,pillar4_idxs, hand_idxs, init_pos_list, init_rot_list,destination_idxs, num_sensors, sensor