...
def create_pillar_asset(gym, sim, pillar_width, pillar_depth, pillar_height, density=100.0):
    # create pillar asset
    asset_options = gymapi.AssetOptions()
    asset_options.density = density  # 设置柱子的密度
    pillar_asset = gym.create_box(sim, pillar_width, pillar_depth, pillar_height, asset_options)  # 修正此处
    return pillar_asset
...
def create_assets(gym, sim, asset_root, table_dims, box_size, ur3e_asset_file):
    ...
    # 4 pillar asset
    pillar1_asset = create_pillar_asset(gym,sim,pillar_width = 0.02,pillar_depth = 0.02,pillar_height=0.15,density=100.0)
    pillar2_asset = create_pillar_asset(gym,sim,pillar_width = 0.02,pillar_depth = 0.02,pillar_height=0.15,density=100.0)
    pillar3_asset = create_pillar_asset(gym,sim,pillar_width = 0.02,pillar_depth = 0.02,pillar_height=0.15,density=100.0)
    pillar4_asset = create_pillar_asset(gym,sim,pillar_width = 0.02,pillar_depth = 0.02,pillar_height=0.15,density=100.0)
...
def create_envs(gym, sim, num_envs, num_per_row, spacing, table_asset, box_asset,destination_asset,pillar1_asset, pillar2_asset, pillar3_asset, pillar4_asset,ur3e_asset, ur3e_dof_props, default_dof_state, default_dof_pos, table_dims, box_size, viewer):
    ...
    ur3e_pose.p = gymapi.Vec3(0.15, 0, 0.3)
    ...
    for i in range(num_envs):
        ...
        # # get global index of box in rigid bodidxy state tensor
