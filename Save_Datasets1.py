...
def main():
    ...
    while not gym.query_viewer_has_closed(viewer):
        ...
        # print("cube pos array",cube_pos_array)

        # # 检查 cube 是否在 destination 上表面的区域内
        # for pos in cube_pos_array:
        #     if (destination_region_min.x <= pos[0] <= destination_region_max.x and
        #             destination_region_min.y <= pos[1] <= destination_region_max.y and
        #             destination_region_min.z <= pos[2] <= destination_region_max.z):
        #         print("arrive destination surface area")
        #         break  # 如果只需要检测一次，找到后就可以退出循环
