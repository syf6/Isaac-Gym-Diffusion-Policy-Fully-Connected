...
for i in range(num_rows):
    for j in range(num_cols):
        ...
        x = j * spacing  # col
        y = i * spacing  # row
        position = gymapi.Vec3(x, y, 0.0) 

        # create new env
        env = gym.create_env(sim, lower + position, upper + position, num_envs)
        envs.append(env)

        # 创建箱子演员
        box_pose = gymapi.Transform()  # 创建新的箱子位置
        box_pose.p = gymapi.Vec3(0.0, 0.0, 0.5)  # 将箱子放在每个环境的上方
        box_actor = gym.create_actor(env, box_asset, box_pose, "box", i * num_cols + j, 0)
        gym.set_rigid_body_color(env, box_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, gymapi.Vec3(0.0, 0.5, 1.0))  # 设置颜色为蓝色
