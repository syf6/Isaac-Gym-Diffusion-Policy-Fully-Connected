...
def main():
    ...
    #  Convert Finger Distance to action "open the gripper" (y: Joystick event: type=1, code=1, value=1) and "close the gripper" (x : Joystick event: type=1, code=0, value=1)  method 2
    previous_distance = 0
    last_distance = 0.05  # 假设初始距离为合拢状态
    last_action = "closed"  # 初始动作状态
    is_closing = False  # 标记是否检测到合拢动作
    is_opening = False  # 标记是否检测到打开动作
    ...
    while not gym.query_viewer_has_closed(viewer):
        ...
        # 获取当前距离
        current_distance = finger_distance
        # 检测夹爪动作
        if current_distance == 0 and current_distance == last_distance: 
            print("Open")
        elif current_distance == 0.05 and current_distance == last_distance:
            print("Close")
        elif current_distance > last_distance and current_distance != 0.05:
            print("X, open to close")
        elif current_distance < last_distance and current_distance != 0:
            print("Y, close to open")
